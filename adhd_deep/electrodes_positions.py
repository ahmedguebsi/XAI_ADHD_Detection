...
from other_features import fd_katz, fd_higuchi
...
#plot connectivity
import mne_connectivity as mne_co
...
def preprocess_eeg_data(raw,info):
    ...
    #bandpass_filtered = filter_data(notch_filtered, sfreq=raw.info['sfreq'], l_freq=4, h_freq=8, method='fir')
    bandpass_filtered= filt_data(notch_filtered, lowcut=4, highcut=8, fs=raw.info['sfreq'], order=6)
...
if __name__=="__main__":
  ...
  print("F4 data",raw.get_data()[5, 28544:29824])
  ...
  #exported_raw=mne.export.export_raw("exported_control_filtered__raw", raw_filtered, fmt='edf', physical_range='auto', add_ch_type=False, overwrite=True,verbose=None)
  #print(type(exported_raw))


  ch_adjacency=mne.channels.find_ch_adjacency(raw_with_dig_pts.info, ch_type='eeg')
  print(type(ch_adjacency))
  print(len(ch_adjacency))
  print(ch_adjacency[1])
  print("ch adjacency",type(ch_adjacency[0]))
  mne.viz.plot_ch_adjacency(info, ch_adjacency[0], ch_names, kind='2d', edit=False)

  epochs = make_fixed_length_epochs(raw_with_dig_pts, duration=4.0, verbose=False)
  ...
  print(con.shape)
  print(type(con))
  print(type(con.get_data()))
  con_reshaped = con.get_data(output='dense')[:, :, 0]

  mne_co.viz.plot_sensors_connectivity(raw_with_dig_pts.info, con_reshaped, picks='eeg', cbar_label='Connectivity')

  covariance = mne.compute_raw_covariance(raw_with_dig_pts,tmin=0, tmax=None)
  covariance.plot(info, exclude=[], show_svd=False, verbose=None)
  ...
  epochs = make_fixed_length_epochs(reconstructed_raw, duration=4.0, verbose=False)
  epochs.load_data().pick_types(eeg=True)
  cov_epoch = mne.compute_covariance(epochs, tmin=0,tmax=None, method=['shrunk', 'empirical'], rank=None, verbose=None)
  cov_epoch.plot(info, exclude=[], show_svd=False, verbose=None)

  print("epoch check shape", epochs.get_data().T.shape)


  print(raw_filtered.get_data().shape)

  dPTE, rPTE = PTE_from_mne(raw_filtered)
  print(dPTE)
  print(rPTE)
  print(rPTE.max(), rPTE.min())
  sns.heatmap(dPTE, cmap='viridis', vmin=0, vmax=1)
  ...
  multitaper = Multitaper(time_series=raw_filtered.get_data().T,
                          sampling_frequency=sampling_frequency,
                          time_halfbandwidth_product=3,
                          time_window_duration=2.0, #look at how connectivity(coherence) evolves over time.
                          #time_window_duration=0.060,
                          #time_window_step=0.060,
                          start_time=time[0])

  print(multitaper.time_window_duration) #The time_window_duration controls the duration of the segment of time the transformation is computed on.
  # This is specified in seconds and automatically set to the entire length of the signal(s)
  print(multitaper.time_window_step) #The time_window_step controls the step size between each time window.
  #control how far the time window is slid. By default, the time window is set to slide the length of the time_window_duration
  # so that each time window is non-overlapping (approximately independent).

  print(multitaper.time_halfbandwidth_product) #The time_halfbandwidth_product controls the number of tapers used in the multitaper spectral estimate.
    # This is specified as the product of the time-bandwidth product and the number of tapers used in the multitaper spectral estimate.
  ...
  print(multitaper.nyquist_frequency)#We can also compute the nyquist frequency, which is the highest resolvable frequency. This is half the sampling frequency.

  fourier_coefficients = multitaper.fft()
  print(fourier_coefficients.shape) #we can use the method fft to get the Fourier coefficients.
  # This will have shape (n_time_windows, n_trials, n_tapers, n_fft_samples, n_signals).

  print(multitaper.frequency_resolution) #The frequency_resolution controls the frequency resolution
  # (The bandwidth is another way to refer to the frequency resolution.) of the multitaper spectral estimate.
    # This is specified in Hz and automatically set to the sampling frequency divided by the number of time samples.

  # Sets up computing connectivity measures/power from multitaper spectral estimate
  connectivity = Connectivity.from_multitaper(multitaper,
                                              expectation_type="time_trials_tapers",#defines the dimensions of the cross-spectrum that are averaged over
                                              # The dimensions you average over is specified by a string with an underscore (_) between the dimensions. defaults to trials_tapers

                                              blocks=1 # breaking this computation up into smaller arrays (blocks)
                                              ) #this formulation, you do not call the .fft method, the class method from_multitaper does this for you

  # Here are a couple of examples
  power = connectivity.power()  # spectral power
  print("power shape",power.shape) # (1,5468,19) we didn't average over time (the expectation_type is "trials_tapers" by default),
  # it is included in the first dimension.
  print(connectivity.frequencies.shape) #(5468,) the frequencies are the same as the frequencies of the multitaper object.
  ...
  plt.show()
  coherence = connectivity.coherence_magnitude() #coherence, a measure of how stable the phase relationships are between the two signals.
  print("coherence shape",coherence.shape)

  plt.plot(connectivity.frequencies, coherence[:, 0, 1]) #The coherence comes in shape (n_frequencies, n_signals, n_signals).
  # Because it is symmetric, we only have to plot the relationship from signal1 to signal2.


  weighted_phase_lag_index = connectivity.weighted_phase_lag_index()
  canonical_coherence = connectivity.canonical_coherence(brain_area_labels)


  # Plot
  fig, axes = plt.subplots(1, 2, figsize=(15, 6))

  axes[0].plot(connectivity.frequencies, connectivity.power().squeeze())
  ...
  #plot_sensors_connectivity(epochs.info,#rPTE,con.get_data(output='dense')[:, :, 0])
  #plt.show()
  ...
  print(epochs.get_data()[20, :, :].T.shape)
  plt.figure(figsize=(10, 10))
  plt.plot(epochs.get_data()[20, :, :].T)
  plt.show()
  estimator.fit(epochs.get_data()[20, :, :].T)
  print(estimator.covariance_.shape)
  from nilearn import plotting

    # Display the covariance
  plotting.plot_matrix(estimator.covariance_, labels=ch_names, colorbar=True,
                          title='Covariance')

  plt.show()

  coords = np.zeros([len(ch_names),3])
  print(coords.shape)
  for i in range(len(ch_names)):
      coords[i,:]=info['chs'][i]['loc'][:3]
      #print(info['chs'][i]['loc'][:3])
  print(type(coords))
  print(coords.shape)
  ...
  print(type(epochs[min_epochs:max_epochs]))
  ...
  print(f'connectivity {method} shape',con.get_data(output="dense").shape)
  ...
  print("sh",ShannonRes)
  print("shannon shape",ShannonRes.shape)
  TsallisRes = tsalisEntropy(eeg_data, bin_min=-200, bin_max=200, binWidth=2, orders=[2])
  print("ts",TsallisRes)
  print("tsallis shape",TsallisRes[0].shape)
  ReyRes = ReyniEntropy(eeg_data, bin_min=-200, bin_max=200, binWidth=2, orders=[2])
  print("rs",ReyRes)
  print("renyi shape",ReyRes[0].shape)
  print(type(eeg_data))
