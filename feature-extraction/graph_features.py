...
network_labels = dict([(network, label) for network, label in
                       zip(labels_df['upload_data.network_name'], labels_df['upload_data.subject_pool'])])
...
features_train, features_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
    adhd_no_hyperactive_features, adhd_no_hyperactive_labels, test_size=0.2)
