...
def hurst_exponent(x):
    return eh.HurstExp(x)

def fisher_information(x):
    return eh.FisherInfo(x)

def correlation_dimension(x):

    return eh.CorrelationDimension(x)

def singular_value_decomposition_entropy(x):
    return eh.SvdEn(x)

def reye_entropy(x):
    return eh.REyEn(x, tau=1, emb_dim=2, r=0.2)[0][-1]

def shannon_entropy(x):
    return eh.ShannonEn(x, base=2)[0][-1]

def tsallis_entropy(x):
    return eh.TsallisEn(x, q=2, base=2)[0][-1]

def log_entropy(x):
    return eh.LogEn(x)

def kraskov_entropy(x):
    return eh.K2En(x, m, tau=1, r=0.2)

def renyi_entropy(x):
    return eh.RenyiEn(x, order=2, base=2)[0][-1]

def permutation_entropy(x):
    return eh.PermEn(x)

def spectral_entropy(x):
    return eh.SpectralEn(x)
